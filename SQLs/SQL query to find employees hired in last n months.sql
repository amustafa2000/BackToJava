...
--select * from Employees where ISNUMERIC(IsNumericTest)=1

select GETDATE();
Select *, 
DATEDIFF(DAY, HireDate, GETDATE()) as DIFF_DAYS,
DATEDIFF(MONTH, HireDate, GETDATE()) as DIFF_MONTH,
DATEDIFF(YEAR, HireDate, GETDATE()) as DIFF_YEAR
FROM Employees
Where DATEDIFF(MONTH, HireDate, GETDATE()) Between 43 and 44
